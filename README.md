...
在 WSL AlmaLinux-8（16 核 / 15.7G 内存）、单机 MongoDB 6.0 下实测（梯度压测）：

| 目标速率 | 实际吞吐 | 丢弃率 |
|---------|---------|--------|
| 10000 条/秒 | 10500 条/秒 | 0.0% |
| 15000 条/秒 | 15749 条/秒 | 20.1% |
| 30000 条/秒 | 31498 条/秒 | 55.6% |

- **接收能力 > 30000 条/秒**（UDP 收包无压力）
- **转发瓶颈约 13000–14000 条/秒**（受 cep-engine 写入速度限制），超限后队列满按 drop 策略丢弃
